 ...
 */
static void H3_verify_process_round(const picnic_instance_t* pp, hash_context* ctx,
 ...
 * Compute challenge
 */
static void H3_process_round(const picnic_instance_t* pp, hash_context* ctx,
...
int impl_sign(const picnic_instance_t* pp, const picnic_context_t* context, uint8_t* sig,
  ...
  for (size_t i = 0; i < num_rounds; ++i) {
...

    {
      ...
      // serialize view and compute commitments
      for (unsigned int j = 0; j < SC_PROOF; ++j) {
    ...
    H3_process_round(pp, &h3_ctx, &round);
  }
...
int impl_verify(const picnic_instance_t* pp, const picnic_context_t* context, const uint8_t* sig,
  ...
  for (size_t i = 0; i < num_rounds; ++i) {
    ...
    H3_verify_process_round(pp, &h3_ctx, &round, original_challenge[i]);
  }
