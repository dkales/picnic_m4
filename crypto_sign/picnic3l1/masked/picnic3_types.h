...
typedef struct maskedRandomTape_t {
  masked_tape_t tape[PICNIC_NUM_PARTIES];
  masked_aux_t aux_bits;
  masked_tape_t parity_tapes;
  uint32_t pos;
  uint32_t aux_pos;
} maskedRandomTape_t;
...
typedef struct maskedMsgs_t {
  masked_view_t msgs[PICNIC_NUM_PARTIES]; // One for each player
  size_t pos;
  int unopened; // Index of the unopened party, or -1 if all parties opened (when signing)
} maskedMsgs_t;
